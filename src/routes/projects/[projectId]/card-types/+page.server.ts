...
export const load: ServerLoad = async ({ params, request }) => {
	...
	console.log('Server load called for card-types, projectId:', projectId);
	...
	try {
		...
		console.log('Card types page loaded successfully for project:', project.id);
		return {
			project,
			team
		};
	} catch (err) {
		console.error('Error loading card types page:', err);
		...
		throw error(500, 'Failed to load card types page');
	}
};