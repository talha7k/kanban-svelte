...
export const load: ServerLoad = async ({ params, request }) => {
	...
	console.log('Server load called for columns, projectId:', projectId);
	...
	try {
		...
		console.log('Columns page loaded successfully for project:', project.id);
		return {
			project,
			team
		};
	} catch (err) {
		console.error('Error loading columns page:', err);
		...
		throw error(500, 'Failed to load columns page');
	}
};