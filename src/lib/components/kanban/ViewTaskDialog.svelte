<script lang="ts">
    ...
    // This single effect is now responsible for initializing or resetting the component's state when the task changes.
    $effect(() => {
        if (task) {
            ...
            // Reset field values based on the current task prop
            ...
            // Also reset the comment input when the task changes
            newComment = "";
        } else {
            // Clear all state if there is no task
            comments = [];
            fieldValues = {};
            newComment = "";
        }
    ...
    // A separate small effect to clear the comment box when the dialog is re-opened for the same task
    $effect(() => {
        if (isOpen) {
            newComment = "";
        }
    ...
    const reporter = $derived(
        users.find((u: UserProfile) => u.id === task?.reporterId),
    );
    const selectedCardType = $derived(
        cardTypes.find((ct: CardType) => ct.id === task?.cardTypeId) || null,
    );

    const dueDateField = $derived(() => {
    ...
    function getFieldDisplayValue(field: any, task: Task): string {
        ...
        const isDueDateField =
            dueDateField() && field.id === dueDateField()!.id;
        if (isDueDateField) {
            value = task.dueDate;
        }
        if (field.type === "fixed") {
    ...
    async function handleSaveFieldValues() {
        ...
        try {
            ...
            if (selectedCardType && selectedCardType.fields) {
                for (const field of selectedCardType.fields) {
                    if (
                        ...
                        if (dueDateField() && field.id === dueDateField()!.id) {
                            // Skip due date field, handle separately
                        } else {
            ...
            const updates: any = { fieldValues: fullFieldValues };
            if (dueDateField()) {
                updates.dueDate = fieldValues[dueDateField()!.id] || null;
            }
        ...
        >
            <DialogHeader class="flex-shrink-0">
                <DialogDescription class="sr-only"
                    >Task details dialog for {task.title}</DialogDescription
                >
                ...
                <div class="flex items-center justify-between space-x-3 mt-1">
                    {#if task.priority !== "NONE"}
                        <Badge
                            ...
                            class={`w-fit ${task.priority === "MEDIUM" ? "bg-accent text-accent-foreground" : ""}`}
                        >
            ...
            <div
                class="flex-1 flex flex-col min-h-0 overflow-y-auto max-h-[88vh]"
            >
                <div class="space-y-4 py-4">
                    {#if task.description}
                        <div>
                            <h3
                                class="font-semibold text-sm mb-1 text-muted-foreground"
                            >
                                Description
                            </h3>
                            <p
                                class="text-sm text-foreground whitespace-pre-wrap bg-muted/30 p-3 rounded-md"
                            >
                                {task.description}
                            </p>
                        </div>
                    ...
                    {#if assignees.length > 0}
                        <div>
                            <h3
                                class="font-semibold text-sm mb-1 text-muted-foreground flex items-center"
                            >
                                <Users class="h-4 w-4 mr-2" />Assignees
                            </h3>
                            <div class="flex flex-wrap gap-2">
                        ...
                        >
                            <div class="flex items-center text-amber-800">
                                ...
                                <span class="text-sm font-medium"
                                    >No Card Type Assigned</span
                                >
                            </div>
                            <p class="text-xs text-amber-700 mt-1">
                                This task was created before card types were
                                implemented. Custom fields are only available
                                for tasks with assigned card types.
                            </p>
                        </div>
                    {:else if selectedCardType && selectedCardType.fields && selectedCardType.fields.length > 0}
                        <div>
                            <h3
                                class="font-semibold text-sm mb-3 text-muted-foreground flex items-center"
                            >
                                <Info class="h-4 w-4 mr-2" />Custom Fields
                            </h3>
                            <div class="flex flex-wrap gap-2">
                                {#each selectedCardType.fields as field (field.id)}
                                    {#if field.name !== "title" && field.name !== "description"}
                                        {#if field.type === "fixed"}
                                            {@const Icon = getFieldTypeIcon(
                                                field.type,
                                            )}
                                            <Badge
                                                variant="secondary"
                                                class="bg-purple-100 text-purple-800 border-purple-200 flex items-center gap-1"
                                            >
                                                <Icon class="h-3 w-3" />
                                                ...
                                                <PopoverTrigger>
                                                    ...
                                                    >
                                                        ...
                                                        {#if isSubmittingFieldUpdate}
                                                            <Loader2
                                                                class="ml-2 h-3 w-3 animate-spin"
                                                            />
                                                        {/if}
                                                    </Badge>
                                                </PopoverTrigger>
                    ...
                    <div class="flex-1 flex flex-col min-h-0">
                        <h3
                            class="font-semibold text-lg mb-2 text-foreground flex items-center flex-shrink-0"
                        >
                            <MessageSquare class="h-5 w-5 mr-2" />Comments ({comments.length})
                        </h3>
            ...
            <DialogFooter
                class="flex-col sm:flex-row gap-3 sm:gap-2 pt-4 border-t flex-shrink-0"
            >
                <Input
